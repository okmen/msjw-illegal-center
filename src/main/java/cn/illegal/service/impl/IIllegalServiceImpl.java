...
public class IIllegalServiceImpl implements IIllegalService {
	 ...
	 * 已注册客户信息同步
	 */
	public String custRegInfoReceive(CustInfoBean custInfo, List<CarInfoBean> carInfo, String openId,
	...
	public BaseBean queryInfoByLicensePlateNo(String licensePlateNo, String licensePlateType,
		...
		if (infos.size() > 0) {
			for (IllegalInfoBean illegalInfoBean : infos) {
				...
				if (StringUtils.isNotBlank(illegalInfoBean.getImgQueryCode())) {
					// illegalImgs =
					// illegalPictureQuery(illegalInfoBean.getImgQueryCode());
		...
		// 屏蔽微信违法图片显示
		/*
		 * if(sourceOfCertification.equals("Z")){ for(IllegalInfoBean
		 * illegalInfoBean : infos){ illegalInfoBean.setImgQueryCode(""); }
		 * }else if(sourceOfCertification.equals("C")){ for(IllegalInfoBean
		 * illegalInfoBean : infos){ illegalInfoBean.setImgQueryCode(""); } }
		 */
	 ...
	 * @throws Exception
	 */
	public List<IllegalInfoBean> queryInfoByDrivingLicenceNo(String drivingLicenceNo, String recordNo, String openId,
			String sourceOfCertification) throws Exception {
		String timeStamp = DateUtil.formatDateTimeWithSec(new Date());
...
//		logger.debug("---" + result.getData());
	 ...
	 * @throws Exception
	 */
	public BaseBean trafficIllegalClaimReg(CustInfoBean custInfo, CarInfoBean carInfo, String openId,
			String sourceOfCertification) throws Exception {
		String timeStamp = DateUtil.formatDateTimeWithSec(new Date());
	...
	public BaseBean trafficIllegalClaimBefore(String licensePlateNo, String licensePlateType, String mobilephone,
			String openId, String sourceOfCertification) throws Exception {
		String timeStamp = DateUtil.formatDateTimeWithSec(new Date());
	 ...
	 */
	@Override
	public IllegalInfoSheet trafficIllegalClaim(String illegalNo, String openId, String sourceOfCertification)
			throws Exception {
		String timeStamp = DateUtil.formatDateTimeWithSec(new Date());
	 ...
	 */
	@Override
	public String toQueryPunishmentPage(String billNo, String licensePlateNo, String mobileNo, String openId,
			String sourceOfCertification) throws Exception {
		...
		try {
			...
			post.setRequestEntity(
					new StringRequestEntity(jsons.toString(), "application/x-www-form-urlencoded", "UTF-8"));
	...
	public String callback(String traffData) throws Exception {
		...
		try {
			...
			url = "https://open.weixin.qq.com/connect/oauth2/authorize?appid=wxc2b699cf2f919b58" + "&redirect_uri="
	 ...
	 * 微信扫码查询信息
	 */
	public String qrCodeToQueryPage(String userName, String traffData, String mobileNo, String openId,
			String sourceOfCertification) throws Exception {
		String url = illegalCache.getPartnerUrl() + "partnerService/qrCodeToQueryPage.do";
		// String
		// url="http://uat.stcpay.com/ysth-traffic-front/partnerService/qrCodeToQueryPage.do";
		PostMethod post = null;
		String timeStamp = DateUtil.formatDateTimeWithSec(new Date());
		String key = "";
		String partnerCode = "";
	 ...
	 * @throws Exception
	 */
	public String toPayPage(String billNo, String licensePlateNo, String mobileNo, String openId,
			String sourceOfCertification) throws Exception {
		...
		try {
			...
			// boolean s=MacUtil.verifyMsgMac(timeStamp, key, bean.getMacAlg(),
			// show1.toString(), mac);
	 ...
	 */
	@Override
	public BaseBean toChangeSubscribe(String snm, String cldbmid, String cczb_id, CustInfoBean custInfo,
			CarInfoBean carInfo, String sourceOfCertification) throws Exception {
...
//			String code = head.get("code").toString();
	 ...
	 */
	@Override
	public BaseBean trafficIllegalAppeal(AppealInfoBean info, String identityCard, String userCode,
			String sourceOfCertification) throws Exception {
		...
		xml.append("<ssly>" + sourceOfCertification + "</ssly>");// 申述来源（ A移动APP
																	// C微信Z支付宝E邮政）
		...
		BaseBean bean = new BaseBean();
		try {
			JSONObject result = WebServiceClient.requestWebService(url, method, jkid, xml.toString(), userid, userpwd,
					key);
		...
		// TransferThirdParty.commitDriverInformationSinglePrintApplicationInterface(applyType,
		// userName, identityCard, mobilephone, sourceOfCertification, url,
		// method, userId, userPwd, key);
	 ...
	 */
	@Override
	public List<AppealInfoBack> trafficIllegalAppealFeedback(String identityCard, String sourceOfCertification)
			throws Exception {
	...
	public Map<String, String> reportingNoParking(ReportingNoParking reportingNoParking) throws Exception {
		...
		String xml = "<?xml version=\"1.0\" encoding=\"gb2312\" ?><REQUEST><HPHM>"
				...
				+ reportingNoParking.getParkingSpot() + "</TCDD><TCZP>" + reportingNoParking.getScenePhoto()
				+ "</TCZP><YHLY>" + reportingNoParking.getSourceOfCertification() + "</YHLY><OPENID>"
				+ reportingNoParking.getOpenId() + "</OPENID></REQUEST>";
		try {
	 ...
	 */
	@Override
	public BaseBean toQueryElectronicReceiptPage(String billNo, String licensePlateNo, String ideNo,
			String sourceOfCertification, String licenseType) throws Exception {
		...
		String data = "{" + "\"idNo\":\"" + "" + "\"," + "\"licenseNum\":\"" + licensePlateNo + "\","
				+ "\"licenseType\":\"" + licenseType + "\"," + "\"billNo\":\"" + billNo + "\"" + "}";
		OpenApiRsp rsp = null;
	...
	public BaseBean receiveMessage(String src, String msgType, String eventType, String sourceOfCertification,
			String giveOpenId) throws Exception {
		String GATEWAY = illegalCache.getPartnerUrl() + "openapi/gateway.do";
		// String GATEWAY
		// ="http://uat.stcpay.com/gov-traffic-front/openapi/gateway.do";
		String APPKEY = "";// illegalCache.getPartnerKey();
		...
		String data = "{" + "\"src\":\"" + src + "\"," + "\"msgType\":\"" + msgType + "\"," + "\"eventType\":\""
				+ eventType + "\"," + "\"openid\":\"" + giveOpenId + "\"" + "}";
		OpenApiRsp rsp = null;
		BaseBean bean = new BaseBean();
	...
	public String szTrafficPoliceElecBillQry(String orderId) throws Exception {
		...
		try {
			 ...
			 * InputStream in = resultStr.getInputStream(); byte[] bytes = new
			 * byte[in.available()]; // 将文件中的内容读入到数组中 in.read(bytes);
			 * 
			 * String strBase64 = Base64Encoder.encode(bytes); //将字节流数组转换为字符串
			 * in.close(); bean.setData(strBase64);
			 */
		} catch (Exception e) {
	...
	public BaseBean evaluateResult(String businessType, String serialNumber, String stars, String sourceOfCertification)
		...
		StringBuffer xml = new StringBuffer();
		xml.append("<?xml version=\"1.0\"  encoding=\"utf-8\"?><REQUEST>").append("<YWLX>").append(businessType)
				.append("</YWLX>").append("<LSH>").append(serialNumber).append("</LSH>").append("<PJXJ>").append(stars)
				.append("</PJXJ>").append("<YHLY>").append(sourceOfCertification).append("</YHLY>")
				.append("</REQUEST>");
	...
	public BaseBean queryEvaluateResult(String serialNumber, String sourceOfCertification) throws Exception {
		...
		StringBuffer xml = new StringBuffer();
		xml.append("<?xml version=\"1.0\"  encoding=\"utf-8\"?><REQUEST>").append("<LSH>").append(serialNumber)
				.append("</LSH>").append("<YHLY>").append(sourceOfCertification).append("</YHLY>").append("</REQUEST>");
		JSONObject json = WebServiceClient.requestWebService(url, method, jkid, xml.toString(), userid, userpwd, key);
		String code = json.getString("CODE");
		String msg = json.getString("MSG");
